...
void quickSort(struct word_keeper *numbers, int left, int right){
	int buffer; // разрешающий элемент
	int l_hold = left;
	int r_hold = right;
	...
	while (left < right){ // пока границы не сомкнутся
		while ((numbers[right].n >= buffer) && (left < right))
			right--; // сдвигаем правую границу пока элемент right больше buffer
		if (left != right){
		...
		while ((numbers[left].n <= buffer) && (left < right))
			left++; // сдвигаем левую границу пока элемент left меньше buffer
	...
	numbers[left].n = buffer; // ставим разрешающий элемент на место
	numbers[left].word = buffer_w;
	buffer = left;
	left = l_hold;
	right = r_hold;
	if (left < buffer) // Рекурсивно вызываем сортировку для левой и правой части массива
