...
int main(int argc, char *argv[]) {
	int max = 1000;
	struct word_keeper *Arr = (struct word_keeper*)calloc(max * sizeof(struct word_keeper));//еще почитать об этой конструкции
	...
	for (int i1 = 0, j1 = 0; ;) {
		...
		if (text >= 'a' && text <= 'z' || text >= 'A' && text <= 'Z') {
			if (start == 0) {
				Arr[i1].word = (char*)calloc(50 * sizeof(char));
				start = 1;
				/*
				get_next_word
			...
			if (start) {
				...
				//девятки memcopy
				//и про функцию сравнения двух строк и про функцию сравнения двух строк  вне зависимости от их регистра (strcmp кажется так)
				if (flag == 999999) {
					j1 = 0;
					flag = -1;
					start = 0;
