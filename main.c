...
int insert(struct word_keeper *Arr, int Arr_size, char *word, int word_size, int left, int right){ 
    if(right-left == 1){
        if(stricmp(word, Arr[left].word) == 0){
            Arr[left].n++;
            return 0;
        }
        else{
            if(stricmp(word, Arr[right].word) == 0){
                Arr[right].n++;
                return 0;
            }
            else{
    ...
    if(stricmp(word, Arr[center].word) < 0){
...
int compare(const void * x1, const void * x2){
    return ( *(int*)(x1+sizeof(char*)) - *(int*)(x2+sizeof(char*)) );              // если результат вычитания равен 0, то числа равны, < 0: x1 < x2; > 0: x1 > x2
}
	...
	for (int i1 = 0, j1 = 0; ;) {
		...
		if (text >= 'a' && text <= 'z' || text >= 'A' && text <= 'Z') {
			...
			if (start) {
                ...
                if(stricmp(buff, Arr[0].word) == 0){//проверка на совпадение с первым словом из словаря
                    Arr[0].n++;
                    flag_new_word = 0;
                }
                else{
                    if(stricmp(buff, Arr[N-1].word) == 0){//проверка на совпадение с последним словом из словаря
                        Arr[N-1].n++;
                        flag_new_word = 0;
                    }
                    else{

                        if(stricmp(buff, Arr[0].word) < 0){//проверка - расположение слова перед словарем
                            ...
                            if(stricmp(buff, Arr[N-1].word) > 0){//проверка - расположение слова после словаря
